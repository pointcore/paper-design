...
- **Path construction**: Make/release even-odd compound paths, join two open paths end to end, close/open paths, simplify anchor counts, outline strokes into filled shapes (paperjs-offset), make/release clipping masks, and a Shape Builder tool (Shift+M drag to unite touched paths, Alt-drag to subtract).
