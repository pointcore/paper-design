...
- **Artboards**: Multiple named page sheets with white-sheet visuals and labels, add (offset beside active) / duplicate (with overlapping artwork, thread links remapped) / delete / rename, click-to-activate with pan, per-board position and size; Save/Open round-trips the set, raster page export targets the active board.
- **Object ops**: Bring to front/back plus stepwise forward/backward, group/ungroup, isolate groups (double-click, banner + Esc to exit), lock/unlock-all, hide/show-all, select same fill/stroke/stroke-width/opacity/blend-mode, lasso loop selection (Q, Shift adds, Alt removes), magic wand (Y, click a fill, Shift-click adds). Locked items are skipped by selection and tools; hidden items never hit-test.
- **Appearance**: Single fill + stroke with caps, joins, miter limit, dash patterns and all 16 canvas blend modes, plus opacity and linear (angle-adjustable via the Gradient tool (G) drag or panel, survives transforms)/radial gradient fills with a stops editor. Swatches panel + bottom color bar (presets, recents, fill/stroke target) apply live from the properties panel to the selection and to subsequently drawn shapes. View > CMYK Preview adds numeric C/M/Y/K readouts with out-of-gamut flags (canvas still renders RGB); fill/stroke accept spot-color placeholder names persisted in project JSON (paints render/export with their RGB preview).
